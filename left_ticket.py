...
def search(from_city, to_city, train_time, ticket_type='ADULT'):
    ...
    try:
        ...
        print('指定起始站点'.decode('utf-8').encode('gbk') + '%s不存在' % from_city)
        sys.exit(-1)
    try:
        to_code   = cities[to_city.decode('gbk')]
    except KeyError:
        print('指定目标站点'.decode('utf-8').encode('gbk') + '%s不存在' % to_city)
        sys.exit(-1)
    ...
    if not ret or ret == -1 or not ret['data'] or len(ret['data']) == 0:
        print('没查询到相关的车次信息'.decode('utf-8').encode('gbk'))
        sys.exit(-1)
    print ('出发日期：'.decode('utf-8').encode('gbk') + ' %s' % ret['data']['queryLeftNewDTO']['start_train_date'])
    print ('车次序号   起始站  出发站  终点站 出发 到达 一等座 二等座  软卧  硬卧  硬座  无座'.decode('utf-8').encode('gbk'))
    for r in ret['data']:
        r = r['queryLeftNewDTO']
        print (u'%s      %s -> %s -> %s %s  %s   %s   %s     %s     %s     %s     %s' %(
            r['station_train_code'],
...
def guide():
    ...
    if city and cities.has_key(city.decode('utf-8')):
        from_city = raw_input('请输入始发城市(输入回车默认为'.decode('utf-8').encode('gbk') + '%s):' % city.decode('utf-8').encode('gbk'))
        if not from_city:
            from_city = city
    else:
        from_city = raw_input('请输入起始站点：'.decode('utf-8').encode('gbk'))
    while True:
        to_city = raw_input('请输入目的站点：'.decode('utf-8').encode('gbk'))
        print to_city
        if to_city:
            break

    dd          = default_date()
    train_time  = raw_input('请输入出发日期(输入回车为'.decode('utf-8').encode('gbk') + '%s):' % dd)
    train_time  = date_format(train_time) if train_time else dd
    ticket_type = '0X00' if get_yn_input('是否成人票'.decode('utf-8').encode('gbk')) else 'ADULT'
